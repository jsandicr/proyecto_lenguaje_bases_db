...
CREATE TABLE TB_TIPO_TAMANO(
   TBTT_ID INTEGER GENERATED ALWAYS AS IDENTITY NOT NULL CONSTRAINT PK_TBTT_ID PRIMARY KEY USING INDEX TABLESPACE TBS_INDEX,
   TBTT_NOMBRE VARCHAR2(50) NOT NULL
) TABLESPACE TBS_DATA STORAGE (INITIAL 1M NEXT 1M PCTINCREASE 0);
...
CREATE TABLE TB_PRODUCTOS(
   ...
   TBTT_ID INTEGER NOT NULL,
   CONSTRAINT PK_TBTP_ID_BDP FOREIGN KEY (TBTP_ID) REFERENCES TB_TIPO_PRODUCTO(TBTP_ID),
   CONSTRAINT PK_TBTT_ID_BDP FOREIGN KEY (TBTT_ID) REFERENCES TB_TIPO_TAMANO(TBTT_ID)
) TABLESPACE TBS_DATA STORAGE (INITIAL 1M NEXT 1M PCTINCREASE 0);
...
CREATE TABLE TB_DETALLE_FACTURA(
   TBDF_ID INTEGER GENERATED ALWAYS AS IDENTITY NOT NULL CONSTRAINT PK_TBDF_ID PRIMARY KEY USING INDEX TABLESPACE TBS_INDEX,
...
create or replace NONEDITIONABLE PACKAGE PKG_USUARIO
	...
	FUNCTION NAVBAR(ROL IN INTEGER,SESION IN INTEGER) RETURN VARCHAR2;
	 ...
	 FUNCTION USUARIO(IDU IN INTEGER)RETURN VARCHAR2;
...
create or replace NONEDITIONABLE PACKAGE BODY PKG_USUARIO
 ...
 IS
    ...
    TEXTO VARCHAR2(30);
    BEGIN
    SELECT TBU_NOMBRE||' '||TBU_APELLIDO1||' '||TBU_APELLIDO2 INTO TEXTO FROM TB_USUARIOS WHERE TBU_ID=IDU;
    RETURN TEXTO;
    EXCEPTION
...
create or replace NONEDITIONABLE PACKAGE PKG_PRODUCTO
    IS
       ...
       PROCEDURE INSERTAR_PRODUCTO(        
                                             ...
                                             TBTT_ID IN TB_TIPO_TAMANO.TBTT_ID%TYPE);
      ...
      PROCEDURE MODIFICAR_PRODUCTO(IDI IN TB_PRODUCTOS.TBP_ID%TYPE,
       ...
       FUNCTION TIPO_PRODUCTO RETURN VARCHAR2;
        ...
        FUNCTION TIPO_TAMANO RETURN VARCHAR2;
...
IS
        BEGIN
            INSERT INTO TB_PRODUCTOS( TBP_NOMBRE, TBP_DESCRIPCION, TBP_PRECIO, TBTP_ID, TBTT_ID)VALUES
            (TBP_NOMBRE,TBP_DESCRIPCION, TBP_PRECIO, TBTP_ID, TBTT_ID);
...
IS
    TEXTO VARCHAR2(200);
    CURSOR BOLSA IS SELECT * FROM TB_TIPO_TAMANO;
    BOLSA_ITEM BOLSA%ROWTYPE;
    BEGIN
        OPEN BOLSA;
        LOOP
            FETCH BOLSA INTO BOLSA_ITEM;
            EXIT WHEN BOLSA%NOTFOUND;
                TEXTO:=TEXTO||'<option value="'||BOLSA_ITEM.TBTT_ID||'">'||BOLSA_ITEM.TBTT_NOMBRE||'</option>';
            END LOOP;
        CLOSE BOLSA;
        RETURN TEXTO;
    END;
...
INSERT INTO TB_TIPO_TAMANO (TBTT_NOMBRE) VALUES ('Pequeño');
INSERT INTO TB_TIPO_TAMANO (TBTT_NOMBRE) VALUES ('Mediano');
INSERT INTO TB_TIPO_TAMANO (TBTT_NOMBRE) VALUES ('Grande');


CREATE OR REPLACE FORCE NONEDITIONABLE VIEW "ADMPIZZA"."VW_PRODUCTOS" ("TBP_ID", "TBP_NOMBRE", "TBP_DESCRIPCION", "TBP_PRECIO", "TBTP_NOMBRE", "TBTT_NOMBRE") AS 
  SELECT TBP_ID, TBP_NOMBRE, TBP_DESCRIPCION, TBP_PRECIO, TBTP_NOMBRE 
FROM TB_PRODUCTOS P 
INNER JOIN  TB_TIPO_PRODUCTO T ON  P.TBTP_ID = T.TBTP_ID
INNER JOIN TB_TIPO_TAMANO TM ON p.tbtt_id = tm.tbtt_id;
